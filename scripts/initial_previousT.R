...
#process fire
fireWeather <- fireWeather[fireWeather$instrument!='VIIRS',]  #exclude satellite which doesn't cover the whole time period
...
colnames(fireSum) <- c('Date','Location','Latitude','Longitude', 'Confidence','Temperature','State','Fires')

write.csv(fireSum,"CorrelationData.csv")
...
selectedData <- fireSum[which(fireSum$State==stateName),]  #just that state

#bubble plot
ggplot(selectedData, aes(x=Date, y=nFire, size = Temperature)) +
